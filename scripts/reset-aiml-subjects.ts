...
async function main() {
  const db = getFirestore();
  const existing = await db.collection("subjects").where("branchId", "==", "aiml").get();
  ...
  for (const s of subjects) {
    const id = `aiml-${s.semester}-${slugifyCode(s.code)}`;
  ...
  await batch.commit();
  console.log("AIML subjects reset and seeded.");
}
